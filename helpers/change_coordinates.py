...
# def inv_pose(origin: GRAPose, absolute: GRAPose) -> ENUPose:
#     """
#     Invert `pose(origin: GRAPose, point: ENUPose)`:
#     Given an absolute GRAPose, return the relative ENUPose in the origin's local frame.
#     """
#     lat0, lon0, z0, h0 = origin
#     lat, lon, z, h_abs = absolute

#     # 1) Global ENU displacement from origin -> absolute
#     #    (use the same geodesy backend you use elsewhere to keep consistency)
#     xg, yg, zg = map(float, geodetic2enu(lat, lon, z, lat0, lon0, z0))  # type: ignore

#     # 2) Undo the origin's heading: rotate by -h0 to get the vector in origin's local frame
#     xr, yr = XY(xg, yg).rotate(-h0)

#     # 3) Altitude and heading differences
#     z_rel = z - z0
#     h_rel = (h_abs - h0) % 360

#     return ENUPose(xr, yr, z_rel, h_rel)
...
# def ENU_to_GRA(origin: GRA, point: ENU) -> GRA:
#     """Convert an ENU point to GRA coordinates relative to a GRA origin."""
#     x, y, z = point
#     lat0, lon0, alt0 = origin
#     lat, lon, alt = map(float, enu2geodetic(x, y, z, lat0, lon0, alt0))  # type: ignore
#     return GRA(lat, lon, alt)
